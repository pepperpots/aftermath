	...
	uint64_t* address_of_element)
{
	...
	// TODO change back to returning void pointer to element
	uint64_t element_address_1 = 0;
	size_t element_index_temp_1 = 0;
	...
	am_event_mapping_for_each_collection_overlapping(m, query, ec) {
			...
			ea, i, element_size, interval_field_offset, query)
		{
			...
			struct am_stack_frame_period* period = 
				(struct am_stack_frame_period*) element_ptr;

			struct am_stack_frame* sf = period->stack_frame;

			if(sf->post_filter_include == 0 || sf->function_symbol->post_filter_include == 0)
				continue;
